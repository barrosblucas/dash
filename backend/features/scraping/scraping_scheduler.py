...
from backend.features.scraping.portal_scrapers import (
    scrape_cargos,
    scrape_contratos,
    scrape_convenios,
    scrape_emendas,
...
class ScrapingScheduler:
    ...
    async def scrape_job(self) -> None:
        ...
        try:
            ...
            result["cargos"] = await scrape_cargos(default_year)
            result["contratos"] = await scrape_contratos(default_year)
            result["convenios"] = await scrape_convenios(default_year)
            result["emendas"] = await scrape_emendas(default_year)
            result["folha"] = await scrape_folha(default_year)
            ...
            logger.exception(
                "Erro durante execução do scraping — ano=%d", default_year,
            )
    ...
    async def trigger_manual(
        ...
        try:
            ...
            logger.exception(
                "Erro no scraping manual — ano=%d data_type=%s", year, data_type,
            )
