...
/** Paleta em tons de vermelho para PieChart (família expense) */
const PIE_COLORS = [
  '#ba1a1a', '#d8270c', '#ad1f0a', '#821707',
  '#df523d', '#e67d6d', '#eea99e', '#570f05',
  '#ff6b35', '#f97316', '#ea580c', '#c2410c',
];
    ...
    <div className="custom-tooltip">
      ...
      <div className="space-y-1">
        ...
        {compararComAnoAnterior && payload[1]?.value !== undefined && (
          <p className="text-label-md text-on-surface-variant">{`${anoAnterior}: ${formatCurrency(payload[1].value)}`}</p>
        )}
      </div>
    </div>
  );
...
function PieCustomTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload: PieTooltipPayload }> }) {
  ...
  return (
    <div className="custom-tooltip">
      <p className="text-label-md text-on-surface-variant mb-1.5 font-medium">{e.name}</p>
      <p className="text-body-sm font-semibold text-expense">{formatCurrency(e.despesas)}</p>
      <p className="text-label-md text-on-surface-variant">{`${e.percentual.toFixed(1)}% do total`}</p>
    </div>
  );
}
...
export default function ExpenseChart({ height = 320, className = '' }: ExpenseChartProps) {
  ...
  if (isLoading) {
    return (
      <div className={`chart-container ${className}`}>
        <div className="mb-4">
          <h3 className="text-title-md font-display text-on-surface">Despesas</h3>
    ...
    <defs>
      ...
      {compararComAnoAnterior && (
        <linearGradient id="colorExpenseAnterior" x1="0" y1="0" x2="0" y2="1">
          <stop offset="5%" stopColor={EXPENSE_PRIMARY} stopOpacity={0.15} />
          <stop offset="95%" stopColor={EXPENSE_PRIMARY} stopOpacity={0} />
        </linearGradient>
      )}
    </defs>
        ...
        return (
          <ResponsiveContainer width="100%" height={height}>
            <BarChart data={cartesianData} margin={margin}>
              ...
              {compararComAnoAnterior && (
                <Bar dataKey="despesasAnterior" fill={EXPENSE_PRIMARY} fillOpacity={0.2}
                  stroke={EXPENSE_PRIMARY} strokeOpacity={0.3} strokeDasharray="5 5" animationDuration={anim} />
              )}
              <Bar dataKey="despesas" fill={EXPENSE_PRIMARY} radius={[4, 4, 0, 0]} animationDuration={anim}>
                {cartesianData.map((e, i) => <Cell key={`cell-${e.mes}`} fill={EXPENSE_PRIMARY} fillOpacity={0.8 + i * 0.015} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        );
        ...
        return (
          <ResponsiveContainer width="100%" height={height}>
            <LineChart data={cartesianData} margin={margin}>
              ...
              {compararComAnoAnterior && (
                <Line type="monotone" dataKey="despesasAnterior" stroke={EXPENSE_PRIMARY} strokeOpacity={0.3}
                  strokeDasharray="5 5" dot={false} animationDuration={anim} />
              )}
              <Line type="monotone" dataKey="despesas" stroke={EXPENSE_PRIMARY} strokeWidth={2}
                dot={{ fill: EXPENSE_PRIMARY, r: 3 }} activeDot={{ r: 5 }} animationDuration={anim} />
            </LineChart>
          </ResponsiveContainer>
        );
      case 'pie':
        return (
          <ResponsiveContainer width="100%" height={height}>
            <PieChart>
              ...
              <Legend verticalAlign="bottom" height={36} formatter={(v: string) => <span className="text-label-md text-on-surface-variant">{v}</span>} />
            </PieChart>
          </ResponsiveContainer>
        );
      case 'area':
        ...
        return (
          <ResponsiveContainer width="100%" height={height}>
            <AreaChart data={cartesianData} margin={margin}>
              {gradientDefs}{sharedGrid}{sharedXAxis}{sharedYAxis}{sharedTooltip}
              {compararComAnoAnterior && (
                <Area type="monotone" dataKey="despesasAnterior" stroke={EXPENSE_PRIMARY} strokeOpacity={0.3}
                  strokeDasharray="5 5" fillOpacity={1} fill="url(#colorExpenseAnterior)" animationDuration={anim} />
              )}
              <Area type="monotone" dataKey="despesas" stroke={EXPENSE_PRIMARY} strokeWidth={2}
  ...
  return (
    <div className={`chart-container ${className}`}>
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h3 className="text-title-md font-display text-on-surface">Despesas</h3>
          <p className="text-body-sm text-on-surface-variant">Execução orçamentária — {anoSelecionado}</p>
        </div>
        <div className="flex items-center gap-4">
          <ChartTypeSelector value={chartType} onChange={setChartType} />
          <div className="text-right hidden sm:block">
            <p className="text-label-md text-on-surface-variant">Total {anoSelecionado}</p>
            <p className="text-headline-sm font-display text-expense">{formatCurrency(totalAno, { compact: true })}</p>
          </div>
        </div>
      </div>
      {renderChart()}
      {isCartesian && compararComAnoAnterior && (
        <div className="mt-4 flex items-center justify-center gap-6 text-label-md text-on-surface-variant">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-expense rounded opacity-80" /><span>{anoSelecionado}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-expense rounded opacity-30" /><span>{anoAnterior}</span>
          </div>
        </div>
      )}
    </div>
