...
import { buscarDiario, importarDiario } from '@/services/diario-oficial-service';
import type { DiarioBuscaItem, DiarioBuscaResponse } from '@/types/diario-oficial';
...
export default function DiarioOficialAdminClient() {
  ...
  const [results, setResults] = useState<DiarioBuscaResponse | null>(null);
  ...
  const performSearch = useCallback(
    async (targetPage: number) => {
      ...
      try {
        const response = await buscarDiario({
          termo: termo || 'LEI',
  ...
  const handleImport = async (item: DiarioBuscaItem) => {
    setImportingId(item.id);
    setImportSuccess(null);
    setImportError(null);

    try {
      const result = await importarDiario({
        diario_id: item.id,
        titulo: item.titulo,
      ...
      setImportError(err instanceof Error ? err.message : 'Erro ao importar publicação.');
    } finally {
      setImportingId(null);
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="font-headline text-2xl font-bold text-primary">Diário Oficial</h2>
        <p className="mt-1 text-sm text-on-surface-variant">
          Busque publicações no Diário Oficial de Mato Grosso do Sul e importe-as como legislação municipal.
        </p>
      </div>
