...
const MES_OPTIONS = [
  ...
  ...MESES.map((nome, idx) => ({ value: idx + 1, label: nome })),
];
...
export default function ReceitasClient() {
  ...
  const { data: detalhamentoData, isLoading: isLoadingDetalhamento } =
    useReceitasDetalhamento(anoSelecionado, mesSelecionado ?? undefined);
  ...
  const summary = useMemo(() => {
    ...
    // Quando mês específico está selecionado, usar dados do detalhamento (mensais).
    // Caso contrário, usar o total anual do endpoint dedicado.
    const arrecadadoCorrenteMensal = topLevelItens
      .filter((item) => item.tipo === 'CORRENTE' && !isDeducao(item.detalhamento))
      .reduce((sum, item) => sum + item.valor_arrecadado, 0);
    const arrecadadoCapital = topLevelItens
      .filter((item) => item.tipo === 'CAPITAL')
      .reduce((sum, item) => sum + (item.valor_arrecadado - item.valor_anulado), 0);

    const arrecadadoCorrente = mesSelecionado != null
      ? arrecadadoCorrenteMensal
      : (totalAnoData?.total_arrecadado ?? 0);

    const arrecadado = tipoReceita === 'CAPITAL' ? arrecadadoCapital : arrecadadoCorrente;
  ...
  }, [detalhamentoData, tipoReceita, totalAnoData, mesSelecionado]);
